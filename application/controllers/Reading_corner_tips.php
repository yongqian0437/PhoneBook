...
class Reading_corner_tips extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $data['include_js'] = 'read_in_progress_tips';

        $this->load->view('templates/header', $data);
        $this->load->view('reading_corner_view_tips.php');
    ...
    public function read($read_num = 0)
    {
        ...
        $data['reading_progress_data'] = $this->reading_corner_model->get_progress_row_tips($user_id);

        $this->load->view('templates/header', $data);
        $this->load->view('read_in_progress_view_tips.php');
        $this->load->view('templates/footer');
    }
    ...
    public function save_progress()
    {
        ...
        $this->reading_corner_model->update_progress_tips($user_id, $progress, $stat);

        // Return a response
        $this->output->set_content_type('application/json')->set_output(json_encode(['status' => 'success']));
    }

    public function save_tips_last()
    {
        $last_progress = $this->input->post('last_progress');
        $user_id = $this->session->userdata('user_id');

        // Update the progress in the database
        $this->reading_corner_model->update_tips_last($user_id, $last_progress);
    ...
    public function get_progress()
    {
        $user_id = $this->session->userdata('user_id');
        $this->load->model('reading_corner_model');
        $progress = $this->reading_corner_model->get_progress_tips($user_id);
    ...
    public function get_saved_progress()
    {
        ...
        $saved_progress = $this->reading_corner_model->get_progress_tips($user_id);
