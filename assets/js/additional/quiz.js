...
function retake_quiz(database) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            window.location.href = base_url + "quiz/retake_quiz/" + database;
        }
    })
}

function generate_result(database) {

    $.ajax({
        url: base_url + "quiz/view_result",
        type: 'POST',
        dataType: 'json',
        data: {
            database: database,
        ...
        success: function (data) {
            ...
            set_data(database);
            set_data_to_result(data);
            $("#exampleModalLongTitle").text("Score " + data.score + "/10");
        },
        error: function (xhr, status, error) {
            Swal.fire({
                icon: 'error',
                title: 'There was an error generating your result, please try again',
            })
        }
    });
...
function set_data_to_result(data) {
    ...
    for (var i = 0; i < 10; i++) {
        ...
        $("#question" + i).text(quiz_questions[i]);
        $("#answer_a" + i).text("A. " + option_a[i]);
        $("#answer_b" + i).text("B. " + option_b[i]);
        $("#answer_c" + i).text("C. " + option_c[i]);
        $("#answer_d" + i).text("D. " + option_d[i]);

        if(user_answer[i] == correct_answers[i]){
            //if user answers correct
            $("#card_body_" + correct_answers[i] + i).addClass("bg-success");
            $("#card_body_" + correct_answers[i] + i).css("color", "white");
            $("#icon"+i).addClass("fas fa-check");
            $("#icon"+i).css("color", "#28a745");
        }
        else {
            $("#card_body_" + correct_answers[i] + i).addClass("bg-success");
            $("#card_body_" + correct_answers[i] + i).css("color", "white");
            $("#card_body_" + user_answer[i] + i).addClass("bg-danger");
            $("#card_body_" + user_answer[i] + i).css("color", "white");
            $("#icon"+i).addClass("fas fa-times");
            $("#icon"+i).css("color", "#dc3545");
        }
    }

}

function set_data(database) {
    ...
    if (database == "quiz_symptom") {
        ...
        correct_answers = [1, 2, 3, 4, 1, 2, 3, 4, 1, 4];
        ...
        correct_answers = [4, 2, 1, 4, 3, 1, 2, 1, 1, 2];
        ...
        correct_answers = [1, 2, 3, 4, 1, 2, 3, 4, 1, 3];
